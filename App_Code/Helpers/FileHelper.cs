    ...
    public static class FileHelper
    {
        private static readonly string _uploadRoot =
            HttpContext.Current?.Server.MapPath(
                ConfigurationManager.AppSettings["UploadPath"] ?? "~/Uploads/")
        ...
        public static string SaveUploadedFile(HttpPostedFile file, int requestId)
        {
            ...
            string safeName = $"{DateTime.Now:yyyyMMddHHmmss}_{SecurityHelper.SanitizeFileName(Path.GetFileName(file.FileName))}";
            string fullPath = Path.Combine(folder, safeName);
            file.SaveAs(fullPath);

            // Return relative path (for DB storage)
            return $"Requests/{requestId}/{safeName}";
        }

        public static string GetPhysicalPath(string relativePath) =>
            Path.Combine(GetUploadRoot(), relativePath.Replace('/', Path.DirectorySeparatorChar));
        ...
        public static void ValidateFile(HttpPostedFile file)
        {
            ...
            if (file.ContentLength > _maxBytes)
                throw new InvalidOperationException($"File exceeds maximum size of {_maxBytes / 1024 / 1024} MB.");
