...
namespace CRMP.BLL
{
    public static class EmailService
    {
        ...
        {
            string subject = $"[{_appName}] Daily Digest — {pendingCount} item(s) awaiting your approval";
            string body = $"You have <strong>{pendingCount}</strong> request(s) pending your approval. Please log in to review them.";
        ...
        {
            string subject = $"[{_appName}] Daily Digest — {poolCount} request(s) in your pool";
            string body = $"There are <strong>{poolCount}</strong> approved request(s) waiting in your tech expert pool.";
        ...
        public static void SendSlaBreachAlert(Request req)
        {
            ...
            string html = BuildEmailHtml(subject, body, $"~/Pages/Employee/RequestDetail.aspx?id={req.RequestId}", "View Request");
        ...
        {
            string fullLink = link.StartsWith("~") || link.StartsWith("/")
                ? link : link;
...
<style>
  ...
  .hdr h1{{margin:0;font-size:18px;font-weight:700;letter-spacing:-.3px}}
  .hdr p{{margin:4px 0 0;font-size:12px;opacity:.8}}
  .body{{padding:28px 32px}}
  .body p{{color:#374151;font-size:15px;line-height:1.6}}
  .btn{{display:inline-block;background:#4F46E5;color:#fff!important;padding:12px 24px;
        border-radius:8px;text-decoration:none;font-size:14px;font-weight:600;margin-top:16px}}
...
<body><div class='wrap'>
  ...
  <div class='body'>
    ...
    <p>{bodyContent}</p>
    <a href='{fullLink}' class='btn'>{btnText}</a>
  </div>
  <div class='ftr'>This is an automated notification from {_appName}. Please do not reply to this email.</div>
</div></body></html>";
        }
    }
}
